...
export default function RoadmapSection() {
  ...
  return (
    <section
      id="roadmap"
      className="py-16 px-6 relative bg-gradient-to-b from-retro-bg via-yellow-50 to-orange-50"
    >
      {/* Background pattern */}
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-gray-700 font-retro max-w-2xl mx-auto">
            Follow this year-by-year guide to prepare systematically and crush
            your placements
          </p>
        </div>
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="text-right">
                    <p className="text-xs font-retro font-bold text-gray-600 uppercase">
                      {item.duration}
                    </p>
                    <p
                      className="text-xs font-retro font-bold"
                      style={{ color: `hsl(var(--${item.color}))` }}
                    >
                      {item.difficulty}
                    </p>
                ...
                {expandedIdx === idx && (
                  ...
                  >
                    <p
                      className="font-bold font-display text-sm mb-3 uppercase"
                      style={{ color: `hsl(var(--${item.color}))` }}
                    >
                      💡 Key Strategies
                    </p>
                    {item.tips.map((tip, tipIdx) => (
                      <p
                        key={tipIdx}
                        className="text-xs font-retro text-gray-700 mb-2"
                      >
                        • {tip}
                      </p>
                    ))}
                ...
                <button
                  className="mt-4 w-full retro-btn text-xs"
                  style={{
                    color: `hsl(var(--${item.color}))`,
                    borderColor: `hsl(var(--${item.color}))`,
                    backgroundColor:
                      expandedIdx === idx
                        ? `hsla(var(--${item.color}), 0.15)`
                        : "transparent",
                  }}
                >
        ...
        <div
          className="retro-border border-4 p-8 bg-gradient-to-r from-purple-50 to-purple-100"
          style={{
        ...
        >
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="font-bold font-display text-retro-purple uppercase mb-2">
                Pro Tip: Consistency Wins
              </p>
              <p className="font-retro text-gray-800">
                The students who crack top companies start preparing from day
                one. Don't wait for your final semester! Build one project,
                solve 5 DSA problems daily, and you'll be unstoppable.
              </p>
            </div>
          </div>
        </div>
